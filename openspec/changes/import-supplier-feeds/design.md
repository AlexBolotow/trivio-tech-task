...
- Текущая договорённость по поведению: worker переводит бизнес-задачу в `in_progress`; Laravel Queue отвечает за доставку и повторы Job; после пяти повторов сверх первой попытки (шесть запусков всего) окончательно неуспешный chunk получает `error`. Собственные heartbeat и retry-счётчики в таблицах Import не хранятся.
...
- Для доставки выбран Laravel Queue с MySQL backend; собственный polling worker не реализуется. `Bus::batch` пока не используется: бизнес-прогресс уже представлен таблицами Import. Timeout и `retry_after` будут согласованы до реализации дочерних Jobs. Outbox не вводится, пока запись Queue и бизнес-данные участвуют в одной MySQL-транзакции.
...
- Import отвечает за бизнес-жизненный цикл загрузки и chunks, а также итоговую оценку полноты импорта. Laravel Queue отвечает за техническую доставку и повторы Jobs.
...
4. Laravel Queue доставляет Job дочерней задачи воркеру; handler переводит бизнес-статус chunk в `in_progress`. Timeout и `retry_after` настраиваются так, чтобы Queue не запускала параллельную повторную доставку до остановки исходного worker.
...
6. Queue может доставить Job повторно после сбоя или timeout. Handler должен быть идемпотентным: повторная обработка chunk приводит к тому же состоянию staging, а уже завершённый `done` chunk пропускается.
...
Выбран Laravel Queue с MySQL backend: parent/child Jobs доставляются через таблицу `jobs`, а `supplier_imports` и `supplier_import_chunks` сохраняют прикладное состояние процесса. Собственные Artisan polling workers не создаются. `Bus::batch` можно рассмотреть позднее, если понадобится отдельное управление группой Jobs; сейчас он дублировал бы часть уже хранимого прогресса.

Собственные `attempts` и heartbeat в таблице chunks не нужны: Queue хранит техническое состояние доставки. Политика timeout/`retry_after` должна быть определена до реализации дочернего Job. Статус `in_progress` означает, что chunk обрабатывается или ожидает очередную Queue-попытку; `done` и `error` — конечные бизнес-результаты.
...
`supplier_import_chunks` — прикладной журнал состава и результата chunks; индекс heartbeat не нужен. Laravel `jobs` и `failed_jobs` остаются техническими таблицами доставки. `job_batches` в первом варианте не используется.
...
- Доменными объектами на первом шаге будут только родительский импорт и дочерний chunk: у них есть бизнес-статусы и допустимые переходы. Технические попытки и heartbeat принадлежат Laravel Queue. Модели не наследуют Eloquent и не знают о БД.
...
SupplierImportRecord / SupplierImportChunkRecord
    → toDomain(record)
    → доменный метод (start, finish, fail)
...
Транзакции принадлежат Application use case. Сохранение данных chunk и перевод его статуса в `done` должны быть атомарны. Создание `supplier_imports` и постановка parent Job тоже выполняются одной транзакцией, если Laravel Queue использует то же подключение MySQL. Длительная обработка файла не должна проходить внутри одной транзакции.
...
- `supplier_import_chunks` уникален по `(supplier_import_id, chunk_number)`; heartbeat-индекс не нужен, так как доставкой управляет Laravel Queue.
...
- Chunk переводится в `in_progress` при начале обработки. Laravel Queue управляет числом повторов; application table хранит только бизнес-статус и временные отметки.
- `done` и `error` терминальны для бизнес-задачи. Повторно доставленный Job для `done` chunk должен завершаться без повторной записи.
...
- Изменение state должно быть идемпотентным с учётом повторной/конкурирующей доставки. Timeout и `retry_after` должны исключать параллельный запуск одного зависшего Job в штатном сценарии.

#### Проверки именно этого участка

- Unit tests на допустимые/запрещённые переходы `SupplierImport` и `SupplierImportChunk`, идемпотентный повтор завершённого chunk и оценку parent по порогу.
...
3. Нужен ли отдельный прикладной журнал ошибок записей/импорта помимо технических Laravel `failed_jobs`.
...
6. Точные значения Laravel Queue `timeout` и `retry_after` для chunk с учётом размера дочерней задачи; число повторов зафиксировано как пять после первой попытки.
...
- Запись статуса и результата chunk выполняется атомарно; длительный разбор файла не удерживает одну транзакцию на весь файл.
...
- Ошибки Job повторяются согласно политике Laravel Queue. Постоянные ошибки валидации записей отправляются в карантин; окончательный отказ Job переводит chunk в `error`, а итоговый статус parent определяется по порогу.
