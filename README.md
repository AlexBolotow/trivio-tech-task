...
| `make reset` | Удалить контейнеры и Compose-managed MySQL volume; внешние volumes сохраняются |
...
После `make up` заполни локальную базу небольшим идемпотентным набором данных:

```bash
make artisan ARGS='db:seed'
```

Seeder добавляет Москву, Санкт-Петербург, по одному отелю в каждом городе и несколько типов номеров с фотографиями. На чистой базе Москва обычно получает `city_id=1`. Запросить её каталог можно так:

```bash
curl --include 'http://localhost:8080/api/v1/catalog/hotels?city_id=1&page=1&per_page=20'
```

Endpoint показывает только активные каталожные данные; цены и доступность в этом ответе не представлены.
...
Персональные настройки Docker Compose можно хранить в игнорируемом Git файле `compose.override.yaml`. В текущем рабочем окружении он добавляет отдельный `mysql-dev` для данных ручной разработки; volume этой базы внешний и переживает `make reset`. Основной Compose MySQL используется для integration-схемы `trivio_testing`.
